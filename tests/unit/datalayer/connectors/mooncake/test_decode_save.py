...
def test_resumed_request_left_to_base():
    assert not should_save_decode_request(
        is_resumed=True, has_new_blocks=True,
        num_computed_tokens=100, prefill_end_tokens=16,
    )


def test_no_new_blocks_skipped():
    assert not should_save_decode_request(
        is_resumed=False, has_new_blocks=False,
        num_computed_tokens=100, prefill_end_tokens=16,
    )
...
def test_chunked_prefill_left_to_base():
    ...
    assert not should_save_decode_request(
        is_resumed=False, has_new_blocks=True,
        num_computed_tokens=8, prefill_end_tokens=16,
    )
...
def test_decode_step_is_handled():
    ...
    assert should_save_decode_request(
        is_resumed=False, has_new_blocks=True,
        num_computed_tokens=16, prefill_end_tokens=16,
    )
    assert should_save_decode_request(
        is_resumed=False, has_new_blocks=True,
        num_computed_tokens=64, prefill_end_tokens=16,
    )
