        ...
        def do_GET(self):
            if self.path == "/metrics":
                body = (
                    ...
                    f'sglang:num_running_reqs{{model_name="m"}} 0.0\n'
                    "# TYPE sglang:num_queue_reqs gauge\n"
                    f'sglang:num_queue_reqs{{model_name="m"}} {stats["waiting"]}\n'
                    "# TYPE sglang:token_usage gauge\n"
                    f'sglang:token_usage{{model_name="m"}} {stats["kv"]}\n'
                ).encode()
...
class MutableStubWorker:
    """Worker whose /metrics render from a dict the test can flip mid-flight."""
...
def _client():
    return TestClient(
        create_app(_scheduler(), metrics_refresh_ms=20, flow_poll_interval_s=0.05)
    )
...
def test_healthy_worker_proxies_straight_through():
...
def test_saturated_worker_queues_until_metrics_recover():
    with MutableStubWorker("a") as worker, _client() as client:
        worker.stats["waiting"] = 9  # saturated from the start
        client.post("/workers", json={"url": worker.url})
        flow_control = client.app.state.flow_control
        assert flow_control.has_plugins()
        # Let the MetricsPoller pick up the saturated snapshot first.
        time.sleep(0.1)

        result = {}

        def issue():
            result["resp"] = client.post("/generate", json={"input_ids": [1, 2, 3]})
        ...
        worker.stats["waiting"] = 0  # capacity returns; watcher re-admits
