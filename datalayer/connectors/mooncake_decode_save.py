...
def should_save_decode_request(
    *,
    ...
    if is_resumed:
        return False
    if not has_new_blocks:
        return False
    return num_computed_tokens >= prefill_end_tokens


class DecodeKVSavingConnector(MooncakeStoreConnector):
    """vllm only saves prompt KV to the store; this also saves decode KV so
    other replicas can reuse generated tokens. Enabled by save_decode_kv."""
    ...
    def __init__(
        ...
        value = vllm_config.kv_transfer_config.kv_connector_extra_config.get(
            "save_decode_kv", False
        )
    ...
    def build_connector_meta(
        ...
        for i, req_id in enumerate(cached.req_ids):
            ...
            if not should_save_decode_request(
                ...
                has_new_blocks=bool(new_block_ids),
