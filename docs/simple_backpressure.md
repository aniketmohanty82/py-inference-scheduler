...
4. **Drain paced by AIMD.** Waiters are admitted one at a time — never as a batch — and every admission re-runs the gate against the latest polled stats, so the polls landing just before and after one admission inform the next (no herd of waiters released onto a single metrics snapshot's best replica). The AIMD `window` sets the *rate*: `window` admissions per poll interval (inter-admission delay = `poll_interval_s / window`), grown additively (`aimd_increase`) while the gate stays open and shrunk multiplicatively (`aimd_decay`) when it shuts. The window seeds from the number of admissible replicas when a parked episode reopens and resets once the queue drains: a fleet-wide capacity release (say KV dropping from 99% to 50%) ramps the drain rate quickly, while a re-saturation halves it immediately.
